...
import com.example.theoraclesplate.ui.auth.viewmodel.AdminAuthState
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun AdminLoginScreenContent(
    state: AdminAuthState,
    onEvent: (AdminAuthEvent) -> Unit
) {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                ...
                actions = {
                    IconButton(onClick = { onEvent(AdminAuthEvent.Logout) }) {
        ...
        Box(
            ...
            Column(
                ...
                OutlinedTextField(
                    value = state.email,
                    onValueChange = { onEvent(AdminAuthEvent.EnteredEmail(it)) },
                    label = { Text("Email") },
                ...
                OutlinedTextField(
                    ...
                    onValueChange = { onEvent(AdminAuthEvent.EnteredPassword(it)) },
                    label = { Text("Password") },
                ...
                Button(
                    onClick = { onEvent(AdminAuthEvent.Login) },
                    modifier = Modifier.fillMaxWidth()
                ) {
                    Text(text = "Login")
                }
