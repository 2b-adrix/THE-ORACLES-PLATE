...
fun PendingSellersScreen(viewModel: PendingSellersViewModel = hiltViewModel()) {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        if (state.isLoading) {
            ...
            Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
                Text(text = "No pending sellers found.")
            }
        } else {
            LazyColumn {
                items(state.pendingSellers) { user ->
                    UserCard(user = user) {
                        Row {
                            Button(onClick = { viewModel.onEvent(PendingSellersEvent.ApproveSeller(user.uid)) }) {
                                Text(text = "Approve")
                            }
                            Spacer(modifier = Modifier.width(8.dp))
                            Button(onClick = { viewModel.onEvent(PendingSellersEvent.DeclineSeller(user.uid)) }) {
                                Text(text = "Decline")
                            }
                        }
                    }
